...
namespace ictbis2
{
    class encryptsn
    {
        ...
        public encryptsn()
        {
            ...
            clsini = new classini();
            tmpreturn = clsini.readdatatxtfile();
        ...
        private void substringSN1forEncrpt()
        {
            this.substring_sn1 = new string[this.sn1.Length]; 
            for (int count = 0; count < this.sn1.Length; count++)
            {
                this.substring_sn1[count] = this.sn1.Substring(count, 1);
            }
        }

        private void substringSN2forEncrpt()
        {
            this.substring_sn2 = new string[this.sn2.Length];
            for (int count = 0; count < this.sn2.Length; count++)
            {
                this.substring_sn2[count] = this.sn2.Substring(count, 1);
            }
        }
        ...
        {
            //Split data 
            this.substringSN1forEncrpt();
            this.substringSN2forEncrpt();
            
            string return_tmp = string.Empty;
            return_tmp = substring_sn1[2] + substring_sn1[6] + substring_sn1[8] + substring_sn1[0] + substring_sn1[3];
            return_tmp += this.hexyear + substring_sn2[0] + substring_sn2[3] + substring_sn2[5];
            return_tmp += this.hexmonth + substring_sn1[9] + substring_sn1[5] + substring_sn1[1] + substring_sn2[2];
            return_tmp += substring_sn1[4] + this.hexmonth + substring_sn1[7];
        ...
        {
            //string hexValue = param.ToString("X");
            //return hexValue;
            ...
            int tmpint = Int32.Parse(param);
            var zz = tmpint.ToString("X");
            return zz;
        }
        public int chgdec(string hexValue)
        {
